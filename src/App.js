    ...
    this.API(this.state.searchText)
      .then(data => {
        ...
        for(let i = 0 ; i < bookss.length ; i ++){
          books.push({
            title: bookss.volumeInfo.title,
            author: bookss.volumeInfo.authors ? bookss.volumeInfo.authors.join(', ') : ' ',
            publisher: bookss.volumeInfo.publisher ? bookss.volumeInfo.publisher : ' ',
            published: bookss.volumeInfo.publishedDate ? bookss.volumeInfo.publishedDate : ' '
          })
        }
        this.setState({
          books: books
